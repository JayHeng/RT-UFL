...
{
    flexspi_update_padsetting_rt1020(config, driveStrength);
}

void flexspi_clock_config(uint32_t instance, uint32_t freq, uint32_t sampleClkMode)
{   
    flexspi_clock_config_rt1020(instance, freq, sampleClkMode);   
}
...
status_t flexspi_set_failsafe_setting(flexspi_mem_config_t *config)
{
    ...
    status = flexspi_set_failsafe_setting_rt1020(config);
...
status_t flexspi_get_max_supported_freq(uint32_t instance, uint32_t *freq, uint32_t clkMode)
{
    ...
    status = flexspi_get_max_supported_freq_rt1020(instance, freq, clkMode);
...
status_t flexspi_get_clock(uint32_t instance, flexspi_clock_type_t type, uint32_t *freq)
{
    ...
    status = flexspi_get_clock_rt1020(instance, type, freq);
...
{
    flexspi_clock_gate_enable_rt1020(instance);
}

void flexspi_clock_gate_disable(uint32_t instance)
{
    flexspi_clock_gate_disable_rt1020(instance);
}

status_t flexspi_nor_write_persistent(const uint32_t data)
{	
    flexspi_nor_write_persistent_rt1020(data);
...
{
    flexspi_nor_read_persistent_rt1020(data);
